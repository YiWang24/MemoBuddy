...
const CardDiary = ({ id, lock, title, handleDelete, handleUnLock,handleLock ,handleButtonClick}) => {
  return (
    <div className="grid grid-cols-[8fr_1fr] p-2 border-2 border-black rounded-md">
      ...
      <div className="flex flex-col gap-2">
        ...
        <button className="text-gray-500 hover:text-gray-700 transition-colors">
          {!lock ? (
            <LockOpenIcon className="h-6 w-6" onClick={()=>handleLock(id)} />
          ) : (
            <LockClosedIcon onClick={()=>handleUnLock(id)} className="h-6 w-6" />
          )}
        </button>
      </div>
    </div>
