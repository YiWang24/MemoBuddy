  ...
  async checkCurrentUser(req, res, next) {
    try {
      ...
      if (
        ...
        console.log(req.session.googleId+"googleId"+req.session.user.email+"email");
        console.log("User is logged in: "+email+"googleId"+googleId);
        res.status(200).json({ message: "User is logged in by google" ,user: { id: req.session.googleId, email: req.session.user.email }});
        console.log("User is logged in by google: ", email);
      } else {
